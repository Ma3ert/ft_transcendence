...
const CostumModal: React.FC<CostumModalProps> = ({ children, value, buttonVariant }) => {
  ...
  return (
    <>
      ...
      <Modal  isOpen={isOpen} onClose={onClose}  variant='default' >
        <ModalContent>
          <ModalCloseButton />
          <ModalBody display={'flex'} justifyContent={'center'} alignItems='center' minH={'auto'}>{children}</ModalBody>
        </ModalContent>
      </Modal>
    </>
  );
