...
const EnviteMessage: React.FC<EnviteMessageProps> = ({}) => {
  ...
  const { joinGameStatus, setJoinGameStatus, gameInviteSender} = useContext(ChatContext);
  ...
  return (
    ...
    >
      <HStack
        ...
        spacing={5}
      >
        <Image src={(gameInviteSender != loggedIndUser.id) ?  "/LightSolidLogo.png" : "/DarkSolidLogo.png" } alt={"envite"} w={8} h={"auto"} />
        <Stack justify={"center"} alignItems={"center"} p={2}>
          <Text
            color={(gameInviteSender != loggedIndUser.id) ? "#5B6171" : "#1D222C"}
            fontSize={"sm"}
            fontWeight={"bold"}
          >
            {(gameInviteSender != loggedIndUser.id) ? activePeer!.username :'you'} Looking for a 1v1 ..
          </Text>
          <Button onClick={()=>{
            setJoinGameStatus! (false)
          }} variant={(gameInviteSender != loggedIndUser.id) ? 'lightGray' : 'darkGray'} >
            {`let's go`}
          </Button>
        </Stack>
      </HStack>
