...
const AvatarUploader: React.FC<AvatarUploaderProps> = ({setAvatarPath, channel, avatarPath}) => {
  ...
  const uploadAvatarMutation = useMutation ({
    mutationFn: async (formData:any) =>await axios.post('http://localhost:3000/chat/channels/avatar', formData, {
    ...
    onSuccess: (res) => {
      if (channel)
        updateAvatar({channel:channel!.id!, avatar:res.data})
      else
        setAvatarPath && setAvatarPath(res.data)
      toast ({
    ...
    <Stack>
        ...
        >
          ...
          <Button borderRadius={'full'} bg='#DC585B' w='20px' position="absolute" onClick={handleButtonClick}
            ...
            >
          <Icon
            as={(channel || file) ? FaPen : AddIcon}
            fontSize='md'
          />
          </Button>
        </Box>
      {file && (<Button variant='ghost' onClick={handleUplaod}>
        <HStack spacing={3}>
          <Text>Upload </Text>
