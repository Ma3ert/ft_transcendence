...
const MemberField: React.FC<MemberFieldProps> = ({ member, members, user }) => {
  ...
  useEffect(() => {}, [Users, user]);
  return (
    <Button variant={"field"} w={"100%"} h="auto">
      <HStack w="100%" h="100%" justify="space-between" alignItems="center">
        ...
        >
          ...
          <Stack
            ...
            alignItems="start"
          >
            <Text fontFamily="visbyRound" color="#D9D9D9" fontSize="sm">
              {user!.username}
            </Text>
            <Text fontFamily="visbyRound" fontSize="xs">
              {member.role.toLowerCase()}
            </Text>
          </Stack>
        </HStack>

        <UserFieldNav member={member!} user={user!} />

        {user!.id != currentUser.user!.id && (
          <OptionsMenu member={member!} user={user!} MembersList={members!} />
        )}
      </HStack>
    </Button>
  );
