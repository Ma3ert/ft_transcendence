...
export class ChatService {
  ...
  async createChannel(owner: string, createChannelDto: createChannelDto) {
    ...
    const channel = await this.prismaService.channel.create({
      data: {
        ...
        members: {
          create: [{ userId: owner, role: Role.OWNER }],
        },
      },
      select: {
        name: true,
  ...
  async kickUser(channel: string, user: string) {
    ...
    if (!isInChannel) throw new ForbiddenException('user is not belong to the same channel');
  ...
  async upgradeUser(owner: string, upgradeuser: string, channel: string) {
    ...
    if (!user) throw new ForbiddenException('user is not belong to the same channel as the owner.');

    if (user.role != Role.MEMBER) throw new ForbiddenException("you can't upgrade an owner or admin user");
  ...
  async downgradeUser(owner: string, downgradeuser: string, channel: string) {
    ...
    if (!user) throw new ForbiddenException('The User is not belong to the same channel as the owner.');
    if (user.role != Role.ADMIN) throw new ForbiddenException("You can't downgrade an owner or member user");
    await this.prismaService.channelUser.update({
      where: {
        userId_channelId: {
          userId: downgradeuser,
  ...
  async banUser(banner: string, banned: string, channel: string) {
    ...
    if (!bannedUser) throw new ForbiddenException('The user to banned is not in the Channel.');
    ...
    if (UserBanned) throw new ForbiddenException('The User is already banned');
  ...
  async unbanUser(unbanner: string, banned: string, channel: string) {
    ...
    if (!user) throw new ForbiddenException('The User is not belong to the same channel.');

    const isbanned = await this.prismaService.channelBan.findFirst({
      where: { userId: banned, channelId: channel },
    });

    if (!isbanned) throw new ForbiddenException('The User to unban is not banned.');
  ...
  async muteUser(muted: string, channel: string) {
    ...
    if (!mutedId || mutedId.role == Role.OWNER) throw new ForbiddenException('You cannot Mute this user');
    ...
    if (isMuted) throw new ForbiddenException('User is already muted');
  ...
  async createChannelInvite(sender: string, receiver: string, channelId: string) {
    ...
    if (senderUser.role === Role.MEMBER && (channel.type == Type.PRIVATE || channel.type == Type.PROTECTED))
      throw new ForbiddenException(
        "The User doesn't have the permission to invite a user to this type of channel.",
      );
    ...
    if (!UserExist) throw new ForbiddenException("The invited User  doesn't exist.");

    const UserInviteExist = await this.prismaService.channelInvite.findFirst({
      where: { receiverId: receiver, channelId: channelId },
    });

    if (UserInviteExist) throw new InternalServerErrorException('The Invite is already sent to the user.');
    ...
    if (userInChannel) throw new ForbiddenException('The User already joined the Channel.');
  ...
  async getUserConversations(user: string) {
    ...
    const conversationsUsers = UserConversations.flatMap((conversation) => {
      const users = [];
      if (conversation.senderId !== user) users.push(conversation.senderId);
      if (conversation.receiverId !== user) users.push(conversation.receiverId);
      return users;
    });
